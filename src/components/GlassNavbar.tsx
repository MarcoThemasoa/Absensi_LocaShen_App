...
interface GlassNavbarProps {
  ...
  /** When true, all items are evenly spaced (1 item per group). Default: false */
  equalSpacing?: boolean;
}

export function GlassNavbar({ leftItems, centerItem, rightItems, equalSpacing = false }: GlassNavbarProps) {
        ...
        {isActive && (
          <motion.span
            ...
            className={`absolute rounded-full bg-white/30 ring-1 ring-white/40 shadow-[0_20px_120px_-50px_rgba(255,255,255,0.35)] backdrop-blur-xl ${
              isCenter
                ? 'opacity-0'
                : 'w-14 sm:w-16 h-14 sm:h-16 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'
            }`}
          />
        )}
        <div
          className={`relative z-10 flex items-center justify-center transition-all duration-200 ease-out ${
            isCenter
              ...
              : 'w-14 sm:w-16 h-14 flex-col gap-1 text-slate-950/95 hover:text-slate-950'
          } ${isActive ? 'scale-105' : 'scale-100'}`}
        >
          <Icon size={isCenter ? 24 : 20} strokeWidth={isActive ? 2.5 : 2} color={isCenter ? 'white' : undefined} />
          {!isCenter && (
            <span className="text-[9px] sm:text-[10px] font-semibold tracking-wide text-center w-full truncate">
              {item.name}
            </span>
          )}
        </div>
        {isCenter && <span className="mt-2 text-[9px] sm:text-[10px] font-bold text-slate-950 tracking-wide">{item.name}</span>}
      </Link>
    );
  };

  return (
    <div className="fixed inset-x-0 bottom-4 z-50 flex justify-center px-2 sm:px-4 md:hidden pointer-events-none">
      <div className="relative w-full max-w-[720px] pointer-events-auto rounded-full border border-white/20 bg-white/10 px-3 sm:px-4 py-3 shadow-[0_30px_80px_-35px_rgba(15,23,42,0.45)] backdrop-blur-3xl">
        ...
        {equalSpacing ? (
          <div className="relative z-10 flex items-center justify-evenly">
            ...
            {centerItem && renderButton(centerItem, true)}
