...
code = code.replace(/import.*from '..\/lib\/utils';/s, importReplacement);

const logicReplacement = `export default function EmployeeHistory() {
  const { user } = useAuth();
...
code = code.replace(/export default function EmployeeHistory\(\) {.*const myRecords = mockAttendance\.filter\(r => r\.userId === user\?\.id\);/s, logicReplacement);

const uiReplacement = `  return (
    <div className="flex flex-col min-h-full bg-gray-50 pb-32">
      <div className="bg-white p-6 shadow-sm border-b border-gray-100 sticky top-0 z-10 flex flex-col gap-4">
        <div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <button 
            ...
            className={cn("px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors", filter === '7' ? 'bg-teal-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}
          >
            7 Hari Terakhir
          </button>
          <button 
            onClick={() => handleFilterChange('30')}
            className={cn("px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors", filter === '30' ? 'bg-teal-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}
          >
            30 Hari Terakhir
          </button>
          <button 
            onClick={() => handleFilterChange('all')}
            className={cn("px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors", filter === 'all' ? 'bg-teal-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}
          >
            Semua
          </button>
        </div>
...
console.log('Patched EmployeeHistory.tsx');
